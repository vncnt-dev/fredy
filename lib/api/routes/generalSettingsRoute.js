...
/**
 * Validate the filesystem root used for automatically downloaded listing media.
 *
 * Relative paths are allowed and resolve beside listings.db. Rejecting an empty value keeps a form
 * clear from silently changing the storage location back to a default the operator did not choose.
 *
 * @param {any} value The raw value from the request body.
 * @returns {string|null} An error message, or null when the value is acceptable.
 */
function validateListingMediaRoot(value) {
  if (typeof value !== 'string' || value.trim().length === 0) {
    return 'listingMediaRoot must be a non-empty path.';
  }
  if (value.includes('\0')) return 'listingMediaRoot must not contain null bytes.';
  return null;
}
...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    if (typeof appSettings.listingMediaRoot !== 'undefined') {
      const error = validateListingMediaRoot(appSettings.listingMediaRoot);
      if (error != null) {
        return reply.code(400).send({ error });
      }
      appSettings.listingMediaRoot = appSettings.listingMediaRoot.trim();
    }
