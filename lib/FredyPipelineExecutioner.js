...
import { preserveCapture, withDetailStatus } from './services/providers/listingCapture.js';
import { archiveListings } from './services/listings/listingArchive.js';
 ...
 * 18) Archive the provider response and listing media
...
class FredyPipelineExecutioner {
  ...
  async _fetchDetails(newListings) {
    ...
    if (!userId || !Array.isArray(enabledProviders) || !enabledProviders.includes(this._providerId)) {
      return newListings.map((listing) => withDetailStatus(listing, 'disabled'));
    }
   ...
   */
  _normalize(listings) {
    return listings.map((listing) => preserveCapture(listing, this._providerConfig.normalize(listing)));
  }
  ...
  /**
   * Archive the provider payload and media for listings that survived every job filter.
   *
   * Archiving is independent of notification channels. The service records individual download
   * failures and contains broader archive errors, so preservation can never be the reason a valid
   * listing notification is withheld.
   *
   * @param {ParsedListing[]} newListings Listings that passed every filter.
   * @returns {Promise<ParsedListing[]>} The same listings, unchanged.
   */
  async _archive(newListings) {
    try {
      return await archiveListings({
        listings: newListings,
        provider: this._providerId,
        jobId: this._jobKey,
      });
    } catch (error) {
      logger.warn('Could not archive new listings', error);
      return newListings;
    }
  }
