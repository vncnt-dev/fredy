...
import {
  ...
  searchCapture,
  withDetailCapture,
  withFailedDetailCapture,
} from '../providers/listingCapture.js';
...
async function getListings(url) {
  ...
  return responseBody.resultListItems
    ...
    .map((expose) => {
      ...
      const images = [image, ...(item?.pictures ?? []).map((picture) => picture?.urlScaleAndCrop)].filter(Boolean);
      return {
        id: item.id,
        price,
        size,
        ...
        image,
        images,
        rawResponse: searchCapture('json', expose),
      };
    });
}
 ...
 * @returns {Promise<ParsedListing>} The listing with a failed detail capture when the API answers with an error
...
async function fetchDetails(listing) {
  ...
  if (!detailed.ok) {
    ...
    return withFailedDetailCapture(listing);
  }
  ...
  const images = (detailBody?.sections ?? [])
    ...
    .map((medium) => medium?.fullImageUrl ?? medium?.imageUrlForWeb ?? medium?.previewImageUrl)
    .filter(Boolean);

  return withDetailCapture(listing, 'json', detailBody, {
    images,
    attachments: attachmentsFromJson(detailBody),
  });
}
