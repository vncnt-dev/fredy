...
export async function readAdvertModificationDate(portal, propertyCode) {
  const detail = await readAdvertDetail(portal, propertyCode);
  const value = detail?.modificationDate?.value;
  return typeof value === 'number' && value > 0 ? value : null;
}

/**
 * Return the complete listing detail response for consumers that need more than its date.
 *
 * @param {Portal} portal
 * @param {string} propertyCode
 * @returns {Promise<any>}
 */
export async function readAdvertDetail(portal, propertyCode) {
  return call(portal, detailPath(portal, propertyCode), {
    method: 'GET',
    query: [
