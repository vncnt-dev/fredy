...
import { mediaFromHtml, withDetailCapture, withFailedDetailCapture } from '../services/providers/listingCapture.js';
...
async function enrichListingFromDetails(listing, browser) {
  ...
  try {
    ...
    if (!html) return withFailedDetailCapture({ ...listing, link: absoluteLink });
    ...
    return withDetailCapture(
      enriched,
      'html',
      html,
      mediaFromHtml(html, ['.galleryimage-element img[data-imgsrc]', '.galleryimage-element img[src]']),
    );
  } catch (error) {
    logger.warn(`Could not fetch Kleinanzeigen detail page for listing '${listing.id}'.`, error?.message || error);
    return withFailedDetailCapture({ ...listing, link: absoluteLink });
  }
}
