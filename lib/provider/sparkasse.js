...
} from '../services/providers/listingCapture.js';
...
async function fetchDetails(listing, browser, extractDetails = puppeteerExtractor) {
  try {
    ...
    if (!html) return withFailedDetailCapture(listing);

    const $ = cheerio.load(html);
    const nextDataRaw = $('#__NEXT_DATA__').text();
    if (!nextDataRaw) return withDetailCapture(listing, 'html', html, mediaFromHtml(html));

    const estate = JSON.parse(nextDataRaw)?.props?.pageProps?.estate;
    if (!estate) return withDetailCapture(listing, 'html', html, mediaFromHtml(html));
    ...
    const pageMedia = mediaFromHtml(html);
    return withDetailCapture(enriched, 'html', html, {
      images: [...pageMedia.images, ...imagesFromJson(estate)],
      attachments: [...pageMedia.attachments, ...attachmentsFromJson(estate)],
    });
  } catch (error) {
