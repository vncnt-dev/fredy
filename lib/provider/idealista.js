...
import { readAdvertDetail, searchListings } from '../services/idealista/search.js';
...
async function fetchDetails(listing) {
  ...
  try {
    const detail = await readAdvertDetail(portal, code);
    const modifiedAt = detail?.modificationDate?.value;
    if (modifiedAt != null) listing.publishedAt = modifiedAt;
    return withDetailCapture(listing, 'json', detail, {
      images: imagesFromJson(detail),
      attachments: attachmentsFromJson(detail),
    });
  } catch (error) {
