...
async function getListings(url) {
  ...
  return rows
    ...
    .map((item) => ({
      ...
      latitude: item.lat,
      longitude: item.lng,
      rawResponse: searchCapture('json', item),
    }));
}
...
async function fetchDetails(listing, browser) {
  try {
    ...
    if (!rawData) return withDetailCapture(listing, 'html', html, mediaFromHtml(html));
    ...
    const pageMedia = mediaFromHtml(html, ['img']);
    return withDetailCapture(enriched, 'html', html, {
      images: [...pageMedia.images, ...imagesFromJson(detailData)],
      attachments: [...pageMedia.attachments, ...attachmentsFromJson(detailData)],
    });
  } catch (error) {
